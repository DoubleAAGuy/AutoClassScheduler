...
def process_schedule(args):
        i, schedule, number_of_students,teachers_file, students_file, output_file, outputsch_file, teacher_class_lists_file = args
        print(f"Processing schedule {i+1}")
        output = mainfile.try_once(schedule, teachers_file, students_file, output_file, outputsch_file, teacher_class_lists_file)
...
def start(loops, teachers_file, students_file, output_file, outputsch_file, teacher_class_lists_file):
    ...
    try:
        ...
        return 'Please add data before running.'
    ...
    with Pool() as pool:
        ...
        args = [
    (i, sch, number_of_students, teachers_file, students_file, output_file, outputsch_file, teacher_class_lists_file)
    ...
    return f'Result: {best_per:.2%} Classes Covered'